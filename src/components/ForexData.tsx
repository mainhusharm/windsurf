  ...
  useEffect(() => {
    ...
    class DataEngine {
        ...
        async fetchYfinancePrice(symbol: string, timeframe: string) {
            const url = `http://127.0.0.1:5009/api/forex-data?pair=${symbol}&timeframe=${timeframe}`;
        ...
        async fetchLatestYfinancePrice(symbol: string) {
            const url = `http://127.0.0.1:5009/api/forex-price?pair=${symbol}`;
            const data = await fetchWithRetry(url);
            if (data && data.price) {
        ...
        async getPrice(symbol: string, timeframe: string) {
            try {
                ...
                return await this.fetchLatestYfinancePrice(symbol);
            } catch (error: any) {
                log(`🟡 yfinance latest price failed for ${symbol}: ${error.message}. Trying historical...`, 'warning');
                try {
                    return await this.fetchYfinancePrice(symbol, timeframe);
    ...
    async function fetchAllForexData(symbols: string[], timeframe: string) {
        try {
            const url = `http://127.0.0.1:5009/api/bulk-forex-price?pairs=${symbols.join(',')}`;
            const data = await fetchWithRetry(url);
    ...
    async function startAnalysis(symbols: string[], timeframes: string[]) {
        ...
        const initialFetch = async () => {
            for (const timeframe of timeframes) {
                const url = `http://127.0.0.1:5009/api/bulk-forex-data?pairs=${symbols.join(',')}&timeframe=${timeframe}`;
